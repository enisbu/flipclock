<script lang="ts">
	// The only operable surface in the app, now a real shadcn drawer (vaul): it
	// slides up without a fade, follows a drag, and dismisses on drag down, tap
	// outside or Escape. The component stays mounted so the exit animation runs;
	// the page controls `open`. On top of the drawer's own behaviour it keeps the
	// idle self-dismiss.
	...
	/** Idle time before the drawer dismisses itself. Long enough to look at all
...
<Drawer.Root bind:open shouldScaleBackground={false}>
	...
	>
		<div
			class="sheet-body mx-auto w-full max-w-[26rem] px-5 pb-[calc(1.25rem+env(safe-area-inset-bottom))]"
		>
...
<style>
	/* Short viewports, which is a phone in landscape: a single column would run
	   against the height cap and bury the slider. The groups sit side by side
	   instead. This shape needs a height media query, which Tailwind has no
	   idiom for. */
