...
One thing to know before you write code: this app refuses features on purpose. It is a clock for a phone that is looked at and never operated, so the interface stays naked. Nothing is visible at rest except the clock, and every setting hides behind a gesture and fades away again. There is no settings gear, no toolbar, no panel. Timers, alarms, weather, calendar, music control, accounts, sync and color pickers are out of scope, and pull requests in those directions will be declined however well they are written. The settings list is exactly five entries: 24 hour time, seconds, date, theme preset, brightness. New runtime dependencies are also declined unless they solve a problem that plain SvelteKit genuinely cannot. Open an issue before building anything large, so nobody wastes an afternoon.
