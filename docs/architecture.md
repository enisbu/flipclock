...
The digits render in `src/lib/fonts/digits.woff2`: Roboto at weight 600, subset to the ten digits, 1.5 KB. It sits under `src/` rather than in `static/` on purpose. SvelteKit never preloads fonts by default, and a face in `static/` is invisible to the build, so it could not be preloaded either. From `src/` vite inlines it into the stylesheet, which is render blocking anyway: no second request, no preload hint, and `font-display: block` waits on nothing extra. Self-hosting makes the rendering predictable; on the target phone it is the face the system stack resolves to anyway, and that stack stays as fallback. All ten digits share one advance width, so the pair never shifts as the minute changes. The plate ratios (`--plate-aspect`, `--digit-of-card`) are optical choices checked against a screenshot, plus a small `translate` nudge: flex centres the line box, not the ink.
